...
<?php
// BỎ HOẶC COMMENT OUT dòng này nếu có:
// do_action( 'flatsome_title_breadcrumbs' );
?>

<?php // THÊM ĐOẠN CODE HIỂN THỊ TIÊU ĐỀ TAXONOMY ?>
<div class="page-title-container container <?php // Thêm class của Flatsome nếu muốn ?>">
    <?php
        // Lấy thông tin của term hiện tại (ví dụ: "Căn hộ")
        $term = get_queried_object();
        if ($term && isset($term->name)) {
            echo '<h1 class="page-title">' . esc_html( $term->name ) . '</h1>';
        } else {
            // Fallback title nếu không lấy được term
            echo '<h1 class="page-title">Dự án Bất động sản</h1>';
        }

        // Hiển thị mô tả của term nếu có
        $description = term_description();
        if ( $description ) {
            echo '<div class="taxonomy-description term-description">' . $description . '</div>';
        }

        // Hiển thị breadcrumbs nếu muốn
